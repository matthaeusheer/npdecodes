...
int main() {
  ...
  // Subproblem d)
  std::cout << "\n -- Subproblem (d)\n";
  std::cout << "Dofs on boundary: " << HandlingDOFs::countBoundaryDofs(lin_dofh)
            << "\n";
  ...
  std::cout << ".. with linear basis functions: I = "
            << HandlingDOFs::integrateLinearFEFunction(lin_dofh, mu) << "\n";
  ...
  std::cout << ".. with quadratic basis functions: I = "
            << HandlingDOFs::integrateQuadraticFEFunction(quad_dofh, zeta)
            << "\n";

  // Subproblem g)
  std::cout << "\n -- Subproblem (g)\n";
  Eigen::VectorXd zeta_constructed =
      HandlingDOFs::convertDOFsLinearQuadratic(lin_dofh, quad_dofh, mu);
